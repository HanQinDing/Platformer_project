...
{
	extern ListAllocator g_listAllocator;
	extern LinearAllocator g_linearAllocator;
	extern bool g_useMyAllocator;
}
...
T* Reserve(const std::initializer_list<T>& initList)
{
	if (CustomAllocators::g_useMyAllocator == true) 
	{
		switch (allocatorType) 
		{
			case LIST_ALLOCATOR: 
			{
				return CustomAllocators::g_listAllocator.Reserve<T, sz>(initList);
				break;
			}
			case LINEAR_ALLOCATOR: 
			{
				return CustomAllocators::g_linearAllocator.Reserve<T, sz>(initList);
				break;
			}
		}
	}
...
T* Reserve()
{
	if (CustomAllocators::g_useMyAllocator == true) 
	{
		switch (allocatorType)
		{
			case LIST_ALLOCATOR: 
			{
				return CustomAllocators::g_listAllocator.Reserve<T, sz>();
				break;
			}
			case LINEAR_ALLOCATOR: 
			{
				return CustomAllocators::g_linearAllocator.Reserve<T, sz>();
				break;
			}
		}
	}
...
T* Reserve(T& object)
{
	if (CustomAllocators::g_useMyAllocator == true) 
	{
		switch (allocatorType) 
		{
			case LIST_ALLOCATOR: 
			{
				return CustomAllocators::g_listAllocator.Reserve<T>(object);
				break;
			}
			case LINEAR_ALLOCATOR: 
			{
				return CustomAllocators::g_linearAllocator.Reserve<T>(object);
				break;
			}
		}
	}
...
void Return(T*& ptr) noexcept
{
	if (CustomAllocators::g_useMyAllocator == true)
	{
		std::cout << "USe mine\n" << CustomAllocators::g_useMyAllocator << "\n";
		switch (allocatorType) 
		{
			case LIST_ALLOCATOR:
			{
				CustomAllocators::g_listAllocator.Return(ptr);
				return;
				break;
			}
			case LINEAR_ALLOCATOR: 
			{
				CustomAllocators::g_linearAllocator.Return(ptr);
				return;
				break;
			}
		}
